#include "engine/bufferscalers/enginebufferscaleresample.h"
...
EngineBufferScaleResample::EngineBufferScaleResample(ReadAheadManager* pReadAheadManager)
...
EngineBufferScaleResample::~EngineBufferScaleResample() {
    SampleUtil::free(m_bufferInt);
}

void EngineBufferScaleResample::setScaleParameters(double base_rate,
        double* pTempoRatio,
        double* pPitchRatio) {
...
void EngineBufferScaleResample::clear() {
    m_bClear = true;
...
// a stretch
double EngineBufferScaleResample::scaleBuffer(
...
SINT EngineBufferScaleResample::do_copy(CSAMPLE* buf, SINT buf_size) {
...
SINT EngineBufferScaleResample::do_scale(CSAMPLE* buf, SINT buf_size) {
    ...
    while (i < buf_size) {
        ...
        if (currentFrameFloor < 0) {
            ...
            if (getOutputSignal().frames2samples(currentFrameFloor) + 1 < m_bufferIntSize) {
                // take floor_sample form the buffer of the previous run
                floor_sample[0] = m_bufferInt[getOutputSignal().frames2samples(currentFrameFloor)];
                floor_sample[1] = m_bufferInt[getOutputSignal().frames2samples(
                                                      currentFrameFloor) +
                        1];
            }
