...
void EngineVinylSoundEmu::process(const CSAMPLE* pIn, CSAMPLE* pOutput, const int iBufferSize) {
    ...
    const CSAMPLE prevQuieting = m_dOldSpeed < thresholdSpeed ? m_dOldSpeed / thresholdSpeed : 1.;
    const bool isDithering = m_dSpeed < ditherSpeed;
    const CSAMPLE curQuieting =  m_dSpeed < thresholdSpeed ? m_dSpeed / thresholdSpeed : 1.;
    ...
    if (wasDithering != isDithering || prevQuieting != curQuieting) {
        for (int i = 0; i < iBufferSize; i += 2) {
            ...
            pOutput[i] = prevQuieting * pIn[i] + dither;
            pOutput[i+1] = prevQuieting * pIn[i+1] + dither;
        }
