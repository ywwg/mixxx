...
void EngineSync::addSyncableDeck(Syncable* pSyncable) {
    VERIFY_OR_DEBUG_ASSERT(!m_syncables.contains(pSyncable)) {
        qWarning() << "EngineSync: already has" << pSyncable;
        return;
    }
...
void EngineSync::setMasterBpm(Syncable* pSource, double bpm) {
    if (kLogger.traceEnabled()) {
        kLogger.trace() << "EngineSync::setMasterBpm" << pSource << bpm;
    }
...
void EngineSync::setMasterParams(
    ...
    if (kLogger.traceEnabled()) {
        kLogger.trace() << "EngineSync::setMasterParams, source is"
                        << pSource->getGroup() << beat_distance << base_bpm
                        << bpm;
    }
    if (pSource != m_pInternalClock) {
